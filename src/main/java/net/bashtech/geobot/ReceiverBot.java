...
public class ReceiverBot extends PircBot {
	...
	protected void onChannelMessage(String channel, String sender,
		...
		// isLive
		if (msg[0].equalsIgnoreCase(prefix + "islive") && isSub) {
			if (msg.length > 1) {
		...
		if (msg[0].equalsIgnoreCase(prefix + "throw") && (isSub)) {
			...
			if (msg.length > 1) {
				...
				{
					for (int i = 1; i < msg.length; i++) {
						if (!sender.equalsIgnoreCase("alefnull")
								&& !sender.equalsIgnoreCase("endsgamer")
								&& (msg[i].toLowerCase().contains("alef")
										|| msg[i].toLowerCase()
												.contains("aief") || msg[i]
										.toLowerCase().contains("ends"))) {
							throwMessage += sender + " ";
						} else
							throwMessage += msg[i] + " ";
					}
