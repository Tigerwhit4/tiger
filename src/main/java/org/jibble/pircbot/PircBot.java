...
public abstract class PircBot implements ReplyConstants {
	...
	public final synchronized void connect(String hostname, int port,
		...
		if (localAddress.equalsIgnoreCase("")) {
			...
			InetAddress localAddr = InetAddress.getByName(BotManager
					.getInstance().getLocalAddress());
			int bindPort = this.findUnusedPort("localhost");
			System.out.println("Attempting to bind to " + localAddr.toString()
					+ ":" + bindPort);
	...
	public void joinChannel(String channel) {
		...
		System.out.println("PircBot: Joined channel " + channel);
	}

	public void joinChannel(String channel, boolean startup) {
		this.sendRawLine("JOIN " + channel);
		if (startup) {
			System.out.println("PircBot: Joining Channel " + channel
					+ " for startup.");
		} else {
			System.out.println("PircBot: Joining Channel " + channel
					+ " NOT for startup.");
	...
	protected void handleLine(String line) {
		...
		if (target == "" && tokenizer.hasMoreTokens()) { // Changed target ==
															// null to target ==
															// ""
			target = tokenizer.nextToken();
		}
		...
		if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0
			...
			this.onMessage(target, sourceNick, sourceLogin, sourceHostname,
					line.substring(line.indexOf(" :") + 2),null);
		} else if (command.equals("PRIVMSG")) {
			// This is a private message to us.
			// System.out.println("DEBUG-TMI: " +
	...
	protected void onUnknown(String line) {
		...
		if (line.startsWith("@") && line.contains("PRIVMSG")) {
			String tagline = line.substring(0, line.indexOf(" "));
			...
			if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0
				...
				if (request.equals("VERSION")) {
					// VERSION request
					this.onVersion(sourceNick, sourceLogin, sourceHostname,
							target);
				} else if (request.startsWith("ACTION ")) {
					// ACTION request
					this.onAction(sourceNick, sourceLogin, sourceHostname,
							target, request.substring(7),tags);
				} else if (request.startsWith("PING ")) {
					// PING request
					this.onPing(sourceNick, sourceLogin, sourceHostname,
					...
					// FINGER request
					this.onFinger(sourceNick, sourceLogin, sourceHostname,
							target);
				...
				// System.out.println("DEBUG: Good message.");
				this.onMessage(target, sourceNick, sourceLogin, sourceHostname,
						line.substring(line.indexOf(" :") + 2),tags);
			} else if (command.equals("PRIVMSG")) {
				// This is a private message to us.
				// System.out.println("DEBUG-TMI: " +
				// line.substring(line.indexOf(" :") + 2));
				...
				this.processMode(target, sourceNick, sourceLogin,
						sourceHostname, mode);
			} else if (command.equals("TOPIC")) {
				// Someone is changing the topic.
				this.onTopic(target, line.substring(line.indexOf(" :") + 2),
	...
	// String sender = displayName.toLowerCase();
	// String channel =
	// line.substring(line.indexOf("#"),line.indexOf(" ",line.indexOf("#")));
	// String message =
	// line.substring(line.indexOf(":",line.indexOf(channel))+1);
	// this.onMessage(channel, sender, sender, sender+".tmi.twitch.tv",
	// message);

	// int pos = line.indexOf(";");
	// String color = line.substring(0,pos);
	// pos= line.indexOf(";",pos+1);
	// String displayName = (line.substring(pos,line.indexOf(";",pos+1)));
	// pos= line.indexOf(";",pos+1);
	// String emotes = line.ind
